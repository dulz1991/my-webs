						...
						$.each(fields, function(index) {
							...
							if (field == 'index') {
								...
								if (field_role.indexOf('1') >= 0) {										
									opHtml += '<a href="javascript:;" index="'+i+'" id="'+item.id+'" onclick="add(this);" class="btn btn-info btn-single btn-sm"><i class="fa fa-plus"></i></a>';
								}
								if (field_role.indexOf('2') >= 0) {
									opHtml += '<a href="javascript:;" index="'+i+'" id="'+item.id+'" onclick="edit(this);" class="btn btn-info btn-single btn-sm"><i class="fa fa-edit"></i></a>';
								}
								if (field_role.indexOf('3') >= 0) {
									opHtml += '<a href="javascript:;" index="'+i+'" id="'+item.id+'" onclick="viewDetail(this);" class="btn btn-info btn-single btn-sm"><i class="fa fa-chevron-eye"></i></a>';
								}
								if (field_role.indexOf('4') >= 0) {
									opHtml += '<a href="javascript:;" index="'+i+'" id="'+item.id+'" onclick="moveUp(this);" class="btn btn-info btn-single btn-sm"><i class="fa fa-chevron-up"></i></a>';
								}
								if (field_role.indexOf('5') >= 0) {
									opHtml += '<a href="javascript:;" index="'+i+'" id="'+item.id+'" onclick="moveDown(this);" class="btn btn-info btn-single btn-sm"><i class="fa fa-chevron-down"></i></a>';
								}
								if (field_role.indexOf('0') >= 0) {
									opHtml += '<a href="javascript:;" index="'+i+'" id="'+item.id+'" onclick="showDeleteDialog(this);" class="btn btn-danger btn-single btn-sm"><i class="fa fa-minus"></i></a>';
								}
								text = opHtml;
								html += '<td>' + text + '</td>';
							} else {

(function($){
	var zp = {
		...
		addhtml:function(obj,pageinit){
			return (function(){
				...
				obj.append('<span>'+'到第'+'<input type="text" class="zxfinput zxfinputPageNo" onkeyup="if(event.keyCode !=37 && event.keyCode != 39){if ((! /^[0-9]+$/ig.test(this.value)) && this.value.length>0){this.value=\'1\';}}" value="'+pageinit.current+'"/>'+'页，'+'</span>');
				obj.append('<span>'+'共'+'<b>'+pageinit.pageNum+'</b>'+'页，'+'</span>');
				/** 消耗时间 */
				if(pageinit.subtractionTime != null){
					if(pageinit.subtractionTime / 60000 > 1){
						obj.append('<span>'+'查询消耗'+'<b>'+pageinit.subtractionTime / 60000+'</b>'+'分，'+'<b>'+pageinit.subtractionTime % 60+'</b>'+'秒，</span>');
					}else{
						obj.append('<span>'+'查询消耗'+'<b>'+Math.round(pageinit.subtractionTime/1000*100)/100+'</b>'+'秒，'+'</span>');
					}
				}else{
					obj.append('<span>'+'查询消耗'+'<b>0</b>'+'秒，'+'</span>');
				}
				
				obj.append('<span>'+'每页'+'<input type="text" class="zxfinput zxfinputPageSize" onkeyup="if(event.keyCode !=37 && event.keyCode != 39){if ((! /^[0-9]+$/ig.test(this.value)) && this.value.length>0){this.value=\'10\';}}" value="'+pageinit.shownum+'"/>条'+'</span>');
	...
	$.fn.createPage = function(options){
		var pageinit = $.extend({
			...
			current : 1, //当前页码
			shownum: 10, //每页条数
			subtractionTime:"subtractionTime",//查询消耗时间
			activepage: "current",
			activepaf: "nextpage",
